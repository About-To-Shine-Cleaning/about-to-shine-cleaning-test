/* =========================================================
   ...
   ATS Payroll (Admin UI) — v4 Dashboard Speed Load + Payroll Adjustments
========================================================= */
...
(() => {
  ...
  async function payrollDashboard(periodId) {
    const qs = periodId ? `period_id=${encodeURIComponent(periodId)}` : "";
    return jsonp(secureUrl("payroll_dashboard", qs));
  }
  ...
  function renderPeriod(p) {
    currentPeriodId = p?.periodId || p?.period || p?.id || p?.currentPeriodId || currentPeriodId || "";
    currentPeriodStatus = p?.status || p?.periodStatus || currentPeriodStatus || "";
    currentPeriodStart = p?.startDate || p?.start || p?.periodStart || currentPeriodStart || "";
    currentPeriodEnd = p?.endDate || p?.end || p?.periodEnd || currentPeriodEnd || "";
  ...
  function normalizeDashboardPayload(res) {
    const period =
      res?.currentPeriod ||
      res?.current ||
      res?.period ||
      {
        periodId: res?.periodId || res?.currentPeriodId || res?.id || currentPeriodId,
        startDate: res?.startDate || res?.periodStart || res?.start || currentPeriodStart,
        endDate: res?.endDate || res?.periodEnd || res?.end || currentPeriodEnd,
        status: res?.status || res?.periodStatus || currentPeriodStatus || "OPEN"
      };
    ...
    const payouts =
      ...
      res?.payrollPayouts ||
      res?.review ||
      { employees: res?.payoutRows || [] };

    const employees =
      res?.employees ||
      res?.employeeRows ||
      res?.payrollEmployees ||
      [];
    ...
    return { period, paymentRows, payouts, employees, jobs, periods };
  }

  function applyDashboardData(res) {
    const data = normalizeDashboardPayload(res);

    renderPeriod(data.period || {});
    setAddJobDateDefault();

    if (Array.isArray(data.employees) && data.employees.length) {
      payrollEmployees = normalizeEmployeeList(data.employees);
      employeeRouteLoaded = true;
      renderAddJobEmployees([]);
    }

    if (Array.isArray(data.jobs) && data.jobs.length) {
      allJobs = normalizeJobList(data.jobs);
      jobsRouteLoaded = true;
    }

    if (Array.isArray(data.periods)) {
      cachedPeriods = data.periods;
      renderPastPayrollPickerFromPeriods(cachedPeriods);
    }

    indexPayrollPaymentRows(data.paymentRows || []);
    renderPayouts(data.payouts || { employees: [] });
    renderPayments(data.paymentRows || [], data.period || {});
  }

  async function loadPeriodLegacy(periodId) {
  ...
  async function loadPeriod(periodId) {
    ...
    try {
      const dash = await payrollDashboard(periodId);
      if (!dash || !dash.ok) throw new Error(dash?.error || "payroll_dashboard failed");
      applyDashboardData(dash);
    } catch (err) {
      console.warn("payroll_dashboard fallback:", err);
      await loadPeriodLegacy(periodId);
    }

    setStatus(`${periodId} loaded ✅`, "ok");
  }

  async function autoloadCurrentPayrollLegacy() {
    ...
    await loadPeriodLegacy(currentPeriodId);
  }

  async function autoloadCurrentPayroll() {
    setStatus("Loading payroll dashboard…");

    try {
      const dash = await payrollDashboard("");
      if (!dash || !dash.ok) throw new Error(dash?.error || "payroll_dashboard failed");
      applyDashboardData(dash);

      if (!currentPeriodId) throw new Error("dashboard_missing_period_id");

      setStatus(`${currentPeriodId} loaded ✅`, "ok");
    } catch (err) {
      console.warn("dashboard load failed, using legacy payroll flow:", err);
      setStatus("Dashboard route unavailable. Loading legacy payroll flow…");
      await autoloadCurrentPayrollLegacy();
    }
  }

  function renderPastPayrollPickerFromPeriods(periods) {
    ...
    pastPayrollSelect.innerHTML = periods.map(p => {
      const id = p.periodId || p.period || p.id || "";
  ...
  async function showPastPayrollPicker() {
    ...
    cachedPeriods = Array.isArray(res.periods) ? res.periods : [];
    renderPastPayrollPickerFromPeriods(cachedPeriods);
  }
  ...
  async function boot() {
    ...
    if (!employeeRouteLoaded || !jobsRouteLoaded) {
      await Promise.allSettled([
        employeeRouteLoaded ? Promise.resolve() : loadPayrollEmployeesList(),
        jobsRouteLoaded ? Promise.resolve() : loadClockJobsList()
      ]);
    }
    ...
    setDebug(`employees=${payrollEmployees.length}; jobs=${allJobs.length}; employeeRouteLoaded=${employeeRouteLoaded}; jobsRouteLoaded=${jobsRouteLoaded}; dashboard=enabled`);
