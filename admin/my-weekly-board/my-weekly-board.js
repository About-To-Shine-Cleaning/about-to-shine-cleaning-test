...
// ✅ Removed Clock Into This Job button
...
function resolveEmployee() {
  ...
  // Keep top Open Clock button working if the HTML still has it.
...
function setViewMode(mode) {
  ...
  if (viewHelp) {
    viewHelp.textContent = currentView === "full"
      ...
      : "My Jobs shows maps and specs. Clock-in is handled from the Clock page only.";
  }
}
...
function renderWeek(rows, weekStart, labelMode = "mine") {
  ...
  if (statusBox) {
    if (labelMode === "full") {
      ...
      statusBox.textContent = activeRows.length
        ? "Tap a client name to view specs. Clock-in is handled from the Clock page only."
        : "No assignments posted for this week yet.";
    }
  }
...
function renderMyJob(job, isPast) {
  ...
  return `
    <div class="my-job-card">
      ...
      ${isPast ? `<span class="my-job-date-lock">Past Day</span>` : `<span class="my-job-date-lock">Use Clock page to clock in</span>`}
    </div>
  `;
}
